...
class PivotKVCache(DynamicCache):
    ...
    def get_prev_temporal_idx(self, layer_idx):
        if len(self.position_cache) <= layer_idx:
            return -1
        else:
            return self.position_cache[layer_idx][0,0,-1]
    ...
    def update(
        ...
        if self.kvcache_compression: # when prefilling visual tokens
            ...
            mrope_section = cache_kwargs.pop('mrope_section')
            ...
            if self.pos_embed_reforge:
                ...
                # # if not os.path.exists(f"/mmu_mllm_hdd/wangxiao24/dev_LongMLLM/compressed_position_ids_{layer_idx}_after.pkl"):
                # #     with open(f"/mmu_mllm_hdd/wangxiao24/dev_LongMLLM/compressed_position_ids_{layer_idx}_after.pkl", 'wb') as f:
                # #         pickle.dump(compressed_position_ids.cpu(), f)

                # NOTE: type 2
                min_temp_id = compressed_position_ids[0].min()
                compressed_position_ids[0] = min_temp_id + ((compressed_position_ids[0] - min_temp_id) * self.compression_ratio).long()
        ...
        else: # when prefilling textual tokens or decoding / kvcache_compression disabled
